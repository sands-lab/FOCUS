...
def _supports_flashinfer_deepseek_topk(device: torch.device) -> bool:
    if not _is_cuda:
        return False
    try:
...
if _is_musa:
    try:
        from mate import moe_fused_gate as musa_moe_fused_gate
    except ImportError:
...
def biased_topk_jit_kernel_impl(
    ...
    if _use_aiter and scoring_func == "sqrtsoftplus" and num_fused_shared_experts == 0:
        ...
        topk_weights, topk_ids = musa_moe_fused_gate(
            gating_output,
            correction_bias,
            topk=topk,
            scoring_func=scoring_func,
...
def biased_grouped_topk_gpu(
    ...
    if (
        ...
        and _supports_flashinfer_deepseek_topk(gating_output.device)
