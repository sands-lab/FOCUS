...
from sglang.srt.dllm.focus import focus_profile_range
...
class FlashInferAttnBackend(AttentionBackend):
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch):
        ...
        if forward_batch.forward_mode.is_decode_or_idle():
            ...
            dllm_sparse = bool(
                (
                    getattr(forward_batch, "dllm_focus_active", False)
                    or getattr(forward_batch, "dllm_delayed_active", False)
                )
                and getattr(forward_batch, "dllm_processing_positions", None)
                is not None
            )
            ...
            profile_name = (
                "flashinfer.init_dllm_sparse_metadata"
                if dllm_sparse
                else "flashinfer.init_metadata"
            )
...
class FlashInferIndicesUpdaterPrefill:
    ...
    def call_begin_forward(
        ...
        if spec_info is None:
            ...
            qo_lens = seq_lens - prefix_lens if query_lens is None else query_lens
