...
class ServerArgs:
    ...
    dllm_enable_delayed_cache: A[
        bool,
        "Enable delayed KV-cache commits during diffusion LLM decoding.",
    ] = False
    dllm_enable_focus: A[
        bool,
        "Enable FOCUS token eviction during diffusion LLM decoding.",
    ] = False
    dllm_focus_alpha: A[
        float,
        "FOCUS retain-count multiplier applied to the running average decoded-token count.",
    ] = 1.0
    ...
    def _handle_dllm_inference(self):
        if self.dllm_algorithm is None:
            return
        if (
            self.dllm_enable_focus or self.dllm_enable_delayed_cache
        ):
            if not self.disable_cuda_graph:
                logger.warning(
                    "Cuda graph is disabled because delayed-cache/FOCUS changes DLLM token shapes during the forward pass."
                )
                self.disable_cuda_graph = True
            # ``cuda_graph_config`` is resolved before this late DLLM hook.
            # Updating only the legacy boolean leaves the already-materialized
            # phase config enabled, so dynamic DLLM batches can be sent to a
            # fixed-shape decode graph. Keep both representations consistent.
            self.cuda_graph_config.decode.backend = Backend.DISABLED
            self.cuda_graph_config.prefill.backend = Backend.DISABLED
