...
class EagerRunner(BaseRunner):
    ...
    def load_batch(
        ...
        load_batch."""
        if envs.SGLANG_EAGER_INPUT_NO_COPY.get() or DllmConfig.from_server_args(
            self.model_runner.server_args
        ) is not None:
            # DLLM can stage token-axis fields with independently shaped views
            # while it advances a block. The eager registry assumes every
            # token-axis input has input_ids' length, which is not true for
            # original FDFO on partially filled batches. Keep DLLM on the
            # ordinary eager path instead of copying into those fixed buffers.
