...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    dllm_focus_avg_tokens: Optional[torch.Tensor] = None
    dllm_focus_block_progress: Optional[torch.Tensor] = None
    dllm_focus_active: bool = False
    dllm_delayed_active: bool = False
    dllm_focus_disabled: bool = False
    dllm_final_forward: bool = False
    dllm_nomask_forward: bool = False
    dllm_processing_positions: Optional[List[torch.Tensor]] = None
    dllm_full_input_ids: Optional[torch.Tensor] = None
    dllm_partial_start_offsets: Optional[torch.Tensor] = None
    dllm_partial_uncached_positions: Optional[torch.Tensor] = None
    dllm_partial_needs_warmup: Optional[torch.Tensor] = None
    dllm_focus_token_sum: Optional[torch.Tensor] = None
    dllm_focus_steps: Optional[torch.Tensor] = None
    ...
    def init_new(
        ...
        *,
        ...
        ret = cls(
            ...
            dimensions=getattr(batch, "dimensions", None),
            return_pooled_hidden_states=getattr(
                batch, "return_pooled_hidden_states", False
            ),
        ...
        if (
            ...
            and batch.dllm_config.enable_delayed_cache
            and any(req.has_dllm_partial_block() for req in batch.reqs)
        ):
            block_size = batch.dllm_config.block_size
            partial_start_offsets = []
            partial_uncached_positions = []
            partial_needs_warmup = []
            focus_token_sum = []
            focus_steps = []
            focus_progress = []
            for req in batch.reqs:
                if req.has_dllm_partial_block():
                    partial_start_offsets.append(getattr(req, "dllm_partial_start", 0))
                    uncached = getattr(req, "dllm_partial_uncached", None)
                    if uncached is None:
                        uncached = [True] * block_size
                    partial_needs_warmup.append(
                        bool(getattr(req, "dllm_partial_needs_warmup", True))
                    )
                    focus_token_sum.append(
                        float(getattr(req, "dllm_focus_token_sum", 0.0))
                    )
                    focus_steps.append(int(getattr(req, "dllm_focus_steps", 0)))
                    focus_progress.append(int(getattr(req, "dllm_focus_progress", -1)))
                else:
                    partial_start_offsets.append(-1)
                    uncached = [True] * block_size
                    partial_needs_warmup.append(True)
                    focus_token_sum.append(0.0)
                    focus_steps.append(0)
                    focus_progress.append(-1)
                partial_uncached_positions.append(uncached)

            ret.dllm_partial_start_offsets = torch.tensor(
                partial_start_offsets, dtype=torch.int32, device=device
            )
            ret.dllm_partial_uncached_positions = torch.tensor(
                partial_uncached_positions, dtype=torch.bool, device=device
            )
            ret.dllm_partial_needs_warmup = torch.tensor(
                partial_needs_warmup, dtype=torch.bool, device=device
            )
            ret.dllm_focus_token_sum = torch.tensor(
                focus_token_sum, dtype=torch.float32, device=device
            )
            ret.dllm_focus_steps = torch.tensor(
                focus_steps, dtype=torch.int32, device=device
            )
            ret.dllm_focus_avg_tokens = torch.where(
                ret.dllm_focus_steps > 0,
                ret.dllm_focus_token_sum
                / torch.clamp(ret.dllm_focus_steps, min=1).to(torch.float32),
                torch.ones(len(batch.reqs), dtype=torch.float32, device=device),
            )
            ret.dllm_focus_block_progress = torch.tensor(
                focus_progress, dtype=torch.int32, device=device
            )
        ...
        use_dllm_block_positions = False
        if batch.dllm_config is not None:
            block_size = batch.dllm_config.block_size
            # The block-local position shortcut is valid only when every
            # request contributes one complete DLLM block. Original FDFO also
            # begins with mixed prompt/block prefill batches, whose token
            # count is not ``batch_size * block_size``; those must use the
            # regular position builder so positions stay aligned with input_ids.
            use_dllm_block_positions = (
                extend_seq_lens is not None
                and all(extend_len == block_size for extend_len in extend_seq_lens)
            )
        if use_dllm_block_positions:
