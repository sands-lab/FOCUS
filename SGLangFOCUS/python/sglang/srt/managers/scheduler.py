...
class Scheduler(
    ...
    def stash_chunked_request(self, req: Req):
        if req.is_dllm() and req.dllm_config.enable_delayed_cache:
            def keep_live_prefix(safe_len: int) -> None:
                req.promote_dllm_committed_prefix(
                    self.req_to_token_pool, safe_len=safe_len
                )
                req.cache_protected_len = 0

            if req.has_dllm_partial_block():
                safe_len = min(req.dllm_block_offset, req.fill_len)
                keep_live_prefix(safe_len)
                return

            full_len = req.fill_len
            safe_len = len(req.origin_input_ids) + len(req.output_ids)
            safe_len = (
                safe_len // req.dllm_config.block_size
            ) * req.dllm_config.block_size
            safe_len = min(safe_len, full_len)

            if safe_len < full_len:
                kv_indices = self.req_to_token_pool.req_to_token[
                    req.req_pool_idx, safe_len:full_len
                ]
                self.token_to_kv_pool_allocator.free(kv_indices)
                req.fill_len = safe_len
                req.kv_committed_len = min(req.kv_committed_len, safe_len)
                req.kv_allocated_len = min(req.kv_allocated_len, safe_len)
        ...
        if self.dllm_config is not None and self.dllm_manager.any_staging_reqs():
            ...
            for req in self.dllm_manager.staging_queue:
                if (
                    ...
                    and not self.dllm_config.enable_delayed_cache
                ):
                    if not req.dllm_incomplete_ids:
                        self.stash_chunked_request(req)
                    # Keep the request row and its KV ownership live through
                    # the next FDFO round.  The original path inserts the
                    # resolved block into the radix cache, but the request can
                    # still own the active block/page until it finishes.  The
                    # completion path releases the row and any final partial
                    # page exactly once.  Unresolved requests already followed
                    # this lifetime because their KV is reused in place.
                else:
                    self.stash_chunked_request(req)
