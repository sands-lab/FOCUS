...
class TpModelWorker(BaseTpWorker):
    ...
    def _forward_batch_generation_dllm(
        ...
        if not self.dllm_algorithm.uses_delayed_cache_scheduler:
            algo_states = None
        ...
        run_output = self.dllm_algorithm.run(self.model_runner, forward_batch)
        if len(run_output) == 3:
            logits_output, next_token_ids, can_run_cuda_graph = run_output
            partial_state = None
        else:
            logits_output, next_token_ids, can_run_cuda_graph, partial_state = (
                run_output
            )
        ...
        if partial_state is not None:
            result.dllm_partial_blocks = partial_state["blocks"]
            result.dllm_partial_uncached_positions = partial_state[
                "uncached_positions"
            ]
            result.dllm_partial_needs_warmup = partial_state["needs_warmup"]
            result.dllm_partial_start_offsets = partial_state["start_offsets"]
            result.dllm_focus_token_sum = partial_state["focus_token_sum"]
            result.dllm_focus_steps = partial_state["focus_steps"]
            result.dllm_focus_block_progress = partial_state["focus_block_progress"]
        return result
