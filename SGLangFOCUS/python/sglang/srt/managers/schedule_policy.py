...
class PrefillAdder:
    ...
    def _uses_delayed_cache_scheduler(self) -> bool:
        return (
    ...
    def _get_running_request_total_token_offset(self, req: Req) -> int:
        if self._uses_delayed_cache_scheduler():
            return 0
        return (
            min(
    ...
    def _get_dllm_remain_tokens(self) -> int:
        ...
        if _rem_tokens <= 0:
            # Keep the one-block admission bound even when the general KV
            # budget is temporarily negative.  Returning the full remaining
            # DLLM budget here can admit a non-page-aligned original-FDFO
            # extend (for example, prompt remainder + one diffusion block),
            # whose partial page is later dropped by radix matching.
            _rem_tokens = min(self.rem_dllm_tokens, self.dllm_block_size)

        return _rem_tokens

    def _get_dllm_req_extend_input_len(self, req: Req, prefix_len: int) -> int:
        max_extend_len = self.dllm_block_size
        if req.has_dllm_partial_block():
            max_extend_len = max(0, len(req.full_untruncated_fill_ids) - prefix_len)

        elif req.dllm_phase in (
            DllmReqPhase.INCOMING_PREFILL,
            DllmReqPhase.STAGING_PREFILL,
        ):
            non_mask_len = max(
                0, len(req.origin_input_ids) + len(req.output_ids) - prefix_len
            )
            max_extend_len = non_mask_len // self.dllm_block_size * self.dllm_block_size
        ...
        if self.rem_total_tokens > 0:
            token_budget = min(token_budget, int(self.rem_total_tokens))

        return token_budget // self.dllm_block_size * self.dllm_block_size

    def _add_dllm_req(self, req: Req, prefix_len: int):
        if self._uses_delayed_cache_scheduler():
            trunc_len = self._get_dllm_req_extend_input_len(req, prefix_len)
        else:
            # Preserve the upstream DLLM admission rule for the original
            # synchronous/FDFO path.
            trunc_len = (
        ...
        if self._uses_delayed_cache_scheduler():
            req.fill_len = req.extend_range.end
    ...
    def add_dllm_staging_req(self, req: Req):
        ...
        if not self._uses_delayed_cache_scheduler():
            ...
            req.set_extend_range(
                len(req.prefix_indices), len(req.prefix_indices) + new_len
            )
            self.can_run_list.append(req)
        ...
        is_staging_prefill = req.dllm_phase == DllmReqPhase.STAGING_PREFILL
        _rem_tokens = (
            self._get_dllm_req_extend_input_len(req, len(req.prefix_indices))
            if is_staging_prefill
        ...
        cand_extend_input_len = (
            self._get_dllm_req_extend_input_len(req, prefix_len)
            if is_staging_prefill
            else max(0, len(req.full_untruncated_fill_ids) - prefix_len)
        )
        if is_staging_prefill:
            new_len = _rem_tokens
        else:
        ...
        # DLLM scheduling bounds the current pass by the active block budget.
        # Reserving AR-style future decode tokens here keeps the active DLLM
        # batch far below max_running_requests for large max_new_tokens.
        self._update_prefill_budget(
            0, req.extend_range.length, 0, req.retracted_stain
        )
    ...
    def add_chunked_req(self, req: Req):
        ...
        self._update_prefill_budget(
            ...
            (
                min(req.sampling_params.max_new_tokens, CLIP_MAX_NEW_TOKENS)
                if not truncated and not self._uses_delayed_cache_scheduler()
                else 0
            ),
            req.retracted_stain,
            mamba_gap_reserve=self._mamba_gap_budget_for_req(req),
    ...
    def add_one_req(
        ...
        delayed_dllm = self._uses_delayed_cache_scheduler()
        prefix_len = len(req.prefix_indices)
        cand_extend_input_len = len(req.full_untruncated_fill_ids) - prefix_len
        dllm_extend_input_len = None
        if delayed_dllm:
            dllm_extend_input_len = self._get_dllm_req_extend_input_len(req, prefix_len)
            if dllm_extend_input_len <= 0:
                return AddReqResult.OTHER
        ...
        budget_extend_input_len = (
            dllm_extend_input_len
            if delayed_dllm
            else cand_extend_input_len
        )
        total_tokens = (
            budget_extend_input_len
            + max_new
            + self.page_size
            + (0 if delayed_dllm else self._mamba_gap_budget_for_req(req))
        )

        # adjusting the input_tokens based on host_hit_length and page_size
        real_input_tokens = (
            budget_extend_input_len - req.host_hit_length
        )
        ...
        with self._lock_node(req.last_node):
            ...
            if delayed_dllm:
                dllm_extend_input_len = self._get_dllm_req_extend_input_len(
                    req, prefix_len
                )
                if dllm_extend_input_len <= 0:
                    return AddReqResult.OTHER
                input_tokens = self.ceil_paged_tokens(dllm_extend_input_len)
            else:
