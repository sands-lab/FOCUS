...
_DLLM_PARTIAL_KV_DEBUG = os.environ.get(
    "SGLANG_DLLM_PARTIAL_KV_DEBUG", ""
).lower() in ("1", "true", "yes", "on")


def _assert_dllm_partial_kv_not_free(req, allocator, partial_kv: torch.Tensor) -> None:
    if not _DLLM_PARTIAL_KV_DEBUG or partial_kv.numel() == 0:
        return

    page_size = allocator.page_size
    partial_pages = torch.unique(partial_kv.to(allocator.device) // page_size)
    free_pages = getattr(allocator, "free_pages", None)
    release_pages = getattr(allocator, "release_pages", None)

    if free_pages is not None and torch.isin(partial_pages, free_pages).any():
        raise RuntimeError(
            "DLLM partial KV page is in allocator free_pages: "
            f"rid={req.rid} pages={partial_pages.detach().cpu().tolist()}"
        )
    if release_pages is not None and torch.isin(partial_pages, release_pages).any():
        raise RuntimeError(
            "DLLM partial KV page is in allocator release_pages: "
            f"rid={req.rid} pages={partial_pages.detach().cpu().tolist()}"
        )
...
class Req(ReqDllmMixin):
    ...
    def init_next_round_input(
        ...
        if tree_cache is not None and (
            not self.is_dllm() or not self.dllm_config.enable_delayed_cache
        ):
            ...
            if self.is_dllm() and not self.dllm_config.enable_delayed_cache:
                self._update_block_offset_for_dllm()

        elif (
            self.is_dllm()
            and self.dllm_config.enable_delayed_cache
            and tree_cache is not None
        ):
            self.last_node = getattr(tree_cache, "root_node", None)
    ...
    def _compute_max_prefix_len(self, input_len: int) -> int:
        ...
        if self.is_dllm():
            max_prefix_len = min(
                max_prefix_len, len(self.origin_input_ids) + len(self.output_ids)
            )
            max_prefix_len = (
                max_prefix_len // self.dllm_config.block_size
    ...
    def _check_repeat_block_finish(self, new_accepted_len: int = 1):
        custom_params = self.sampling_params.custom_params
        if not isinstance(custom_params, dict):
            return False

        block_window = custom_params.get("repeat_block_window")
        if block_window is None:
            return False

        try:
            block_window = int(block_window)
            repeats = int(custom_params.get("repeat_block_threshold", 3))
        except (TypeError, ValueError):
            return False

        if block_window <= 0 or repeats < 2:
            return False

        total = len(self.output_ids)
        if total < repeats:
            return False

        start = max(1, total - max(new_accepted_len, 1) + 1)
        for end in range(start, total + 1):
            if self._has_repeated_block_at_end(end, block_window, repeats):
                self.finished_reason = FINISH_MATCHED_STR(matched="repeat_block")
                self.finished_len = end
                return True

        return False

    def _has_repeated_block_at_end(
        self, end: int, block_window: int, repeats: int
    ) -> bool:
        if end < repeats:
            return False

        max_block = min(block_window, end // repeats)
        if max_block <= 0:
            return False

        min_block = 8 if max_block >= 8 else 1
        for block_size in range(max_block, min_block - 1, -1):
            tail_start = end - block_size
            tail = self.output_ids[tail_start:end]
            repeated = True
            for idx in range(2, repeats + 1):
                block_start = end - block_size * idx
                if block_start < 0:
                    repeated = False
                    break
                if self.output_ids[block_start : block_start + block_size] != tail:
                    repeated = False
                    break
            if repeated:
                return True
    ...
    def update_finish_state(self, new_accepted_len: int = 1):
        ...
        if new_accepted_tokens:
            ...
            if self._check_repeat_block_finish(new_accepted_len):
                return
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def prepare_for_extend(self):
        ...
        if self.is_dllm() and self.dllm_config.enable_delayed_cache:
            pt = 0
            for req, input_len in zip(reqs, input_id_lens):
                partial_kv = getattr(req, "dllm_partial_kv_indices", None)
                if partial_kv is not None:
                    if input_len != len(partial_kv):
                        raise RuntimeError(
                            "DLLM partial block KV length mismatch: "
                            f"input_len={input_len}, kv_len={len(partial_kv)}"
                        )
                    page_size = self.tree_cache.page_size
                    prefix_len = len(req.prefix_indices)
                    reused = partial_kv.to(
                        device=self.device, dtype=out_cache_loc.dtype, non_blocking=True
                    )
                    _assert_dllm_partial_kv_not_free(
                        req, self.token_to_kv_pool_allocator, reused
                    )
                    existing_page_tokens = (
                        min(input_len, ceil_align(prefix_len, page_size) - prefix_len)
                        if page_size > 1
                        else 0
                    )
                    newly_allocated = out_cache_loc[
                        pt + existing_page_tokens : pt + input_len
                    ]
                    if _DLLM_PARTIAL_KV_DEBUG and newly_allocated.numel() > 0:
                        reused_pages = torch.unique(reused // page_size)
                        new_pages = torch.unique(newly_allocated // page_size)
                        if torch.isin(reused_pages, new_pages).any():
                            raise RuntimeError(
                                "DLLM partial KV page was reallocated before reuse: "
                                f"rid={req.rid} "
                                f"partial_pages={reused_pages.detach().cpu().tolist()} "
                                f"new_pages={new_pages.detach().cpu().tolist()}"
                            )
                    self.token_to_kv_pool_allocator.free(newly_allocated)
                    _assert_dllm_partial_kv_not_free(
                        req, self.token_to_kv_pool_allocator, reused
                    )
                    out_cache_loc[pt : pt + input_len] = reused
                    self.req_to_token_pool.write(
                        (
                            req.req_pool_idx,
                            slice(len(req.prefix_indices), req.fill_len),
                        ),
                        reused.to(torch.int32),
                    )
                pt += input_len
