...
def maybe_cache_unfinished_req(req: Req, tree_cache: BasePrefixCache, **kwargs):
    if (
        (req.is_dllm() and req.dllm_config.enable_delayed_cache)
...
def _release_overallocated_kv_indices(
    ...
    # Both delayed-cache DLLM and original FDFO can finish part-way through
    # the last diffusion block.  That block has already been allocated, but
    # its unaccepted suffix must be released at request completion.
    custom_dllm = req.is_dllm()
    if (
        spec_algo is None
