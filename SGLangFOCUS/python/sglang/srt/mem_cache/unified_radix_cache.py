        ...
        while len(key) > 0 and child_key in node.children:
            ...
            if node.evicted:
                ...
                if dup_start < consumed_from:
                    free_value = value_slice[dup_start:consumed_from]
                    cached_value = node.component_data[BASE_COMPONENT_TYPE].value
                    if cached_value is not None:
                        cached_slice = cached_value[dup_start:consumed_from]
                        if cached_slice.numel() > 0:
                            if self.page_size > 1:
                                free_pages = torch.unique(free_value // self.page_size)
                                cached_pages = torch.unique(
                                    cached_slice // self.page_size
                                )
                                fresh_mask = ~torch.isin(free_pages, cached_pages)
                                free_pages = free_pages[fresh_mask]
                                if free_pages.numel() == 0:
                                    free_value = None
                                else:
                                    page_offsets = torch.arange(
                                        self.page_size,
                                        dtype=free_pages.dtype,
                                        device=free_pages.device,
                                    )
                                    free_value = (
                                        free_pages[:, None] * self.page_size
                                        + page_offsets[None, :]
                                    ).reshape(-1)
                            else:
                                fresh_mask = ~torch.isin(free_value, cached_slice)
                                free_value = free_value[fresh_mask]

                    if free_value is not None and free_value.numel() > 0:
                        self.token_to_kv_pool_allocator.free(free_value)
